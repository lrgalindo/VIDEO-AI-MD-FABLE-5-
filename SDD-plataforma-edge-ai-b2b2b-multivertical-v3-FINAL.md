# System Design Document (SDD) v3.3 — FINAL
...
> el material público de Agrex.ai vigente a la misma fecha. Toda afirmación sobre
> disponibilidad de extensiones en el hosting destino (Supabase) fue verificada
> contra su documentación oficial vigente a la misma fecha. Los cambios de cada
...
**v3.3-FINAL (esta iteración):** cierre de portabilidad de hosting y barrido de
...
* **Facturación / Liquidación de Comisiones de Reseller (nuevo, v3.3):** mismo
  principio ya aplicado a la relación Asset Owner→Partner. La plataforma provee
  **atribución** — reporting de qué Tenants operan bajo qué Reseller (`tenants.
  reseller_id`) — pero **no** liquida ni cobra la comisión del Reseller. Ese acuerdo
  (porcentaje, contrato, pago) es externo a la plataforma. Decisión deliberada para
  no convertirnos en un motor de facturación cruzada de tres bandas en el MLP.
...
| **Service Account (Edge Gateway)** | Sistema | Sistema físico en sucursal/bodega. | Autenticación máquina a máquina por **certificado cliente mTLS** (Sección 8.7, actualizado v3.3 — reemplaza el "JWT/API Key" genérico anterior) para publicar telemetría y para solicitar al Model Manager la descarga del modelo del vertical asignado. Revocable server-side contra `edge_gateways.status` en cada handshake. |
...
#### Flujo 6: Referido de Tenant por un Reseller (nuevo, v3.3)

Cierra el ciclo de vida comercial del rol Reseller Admin, ya definido en la Sección
4 pero sin flujo propio hasta ahora.
...
(`cert_serial`, `cert_expires_at`, `replaced_edge_gateway_id`, ampliación del enum
`status`): ver el DDL en la Sección 8.7.
...
#### 7.3 Decisiones Build-vs-Buy (nueva, v3.3)
...
| --- | --- | --- | --- |
| Orquestación/sandboxing de agentes | **Buy — Claude Managed Agents** | No es diferenciador (lo es el aislamiento B2B2B); construirlo son meses de infra sin ventaja competitiva. | Dependencia de la disponibilidad y pricing de un proveedor (mitigado: fallback a Messages API, Sección 12.4). |
| Auth / Identity Provider | **Buy — Supabase Auth** | Gratis dentro del mismo proyecto de DB; se integra nativamente con el patrón de GUCs de la Sección 8.2. | Acoplamiento a Supabase (mitigado: es solo IdP; el RLS es portable). |
...
| Canal WhatsApp (Motor de Acciones) | **Buy — Meta Cloud API directo, sin BSP** | Meta lo exige (no hay "build"); WhatsApp es el canal dominante en CENAM. | Evitar la capa de costo de un BSP mientras el volumen de alertas es bajo; re-evaluar si el volumen sube. |
...
| CA para mTLS del Edge (Sección 8.7) | **Build — CA interna ligera** | Es parte del mecanismo de seguridad diferenciador; una CA pública no encaja con revocación contra estado en DB. | Operar rotación de CA raíz (documentado, fuera del MLP). |
...
| **QA** | Instancia/proyecto Postgres separado | Corre la suite de aislamiento (pgTAP) contra un clon real de la topología de extensiones (`pg_partman`, `pgcrypto`, `citext`, RLS) sin compartir recursos con Staging. |
| **Staging** | Instancia/proyecto Postgres separado, con IaC idéntico a Prod | Debe ser un espejo fiel de Prod (misma versión de Postgres, mismas extensiones, mismas políticas RLS y misma configuración de particionamiento) — es el último gate antes de producción. |
...
#### 8.7 Credenciales del Edge Gateway: mTLS, Rotación y Revocación (nueva, v3.3)

Versiones anteriores mencionaban "JWT/API Key" para la Service Account del Edge
Gateway (Sección 4) sin especificar almacenamiento, rotación ni revocación — un gap
de seguridad para un dispositivo que vive físicamente en la sede del cliente y es,
por tanto, el punto más expuesto de la arquitectura. Decisión v3.3: **autenticación
por certificado cliente mTLS**, no JWT de larga duración.

**Por qué mTLS y no JWT:** un JWT de larga duración almacenado en la sede es un
secreto exfiltrable que sobrevive hasta su expiración; uno de corta duración exige
un flujo de refresh que se rompe en los cortes de internet típicos de CENAM. Un
certificado cliente mTLS: (a) sobrevive reinicios sin flujo de refresh (vive en el
volumen de Docker); (b) funciona idéntico en Windows/Linux/macOS; (c) permite
revocación server-side inmediata contra el estado en DB, sin esperar expiración; y
(d) la clave privada nunca viaja por la red (se genera localmente; solo el CSR sale
al canjear el código de activación).

**Emisión:** al canjear el código de activación de un solo uso (Flujo 1, paso 4, o
Flujo 7 para reemplazo), el Edge Gateway genera su par de claves localmente y envía
un CSR; el backend actúa como CA interna y emite el certificado cliente, registrando
su serial y expiración. Columnas nuevas en `edge_gateways`:
...
-- Ampliación del enum de status para soportar baja/reemplazo (Flujo 7).
...
'decommissioned')`, la conexión se rechaza aunque el certificado siga vigente. Esto
permite dar de baja un gateway robado o comprometido de inmediato, sin depender de
CRL/OCSP ni de la expiración del certificado. (El chequeo contra DB en cada
handshake es barato: un lookup por índice único sobre `cert_serial`.)

**Rotación automática antes de expiración:** los certificados se emiten con vigencia
acotada (ej. 90 días). El Edge Gateway llama proactivamente a un endpoint de
renovación (`POST /v1/edge/cert/renew`, autenticado con el certificado aún vigente)
cuando le quedan ~2 semanas — obtiene un certificado nuevo sin intervención manual y
sin ventana de desconexión. Si el gateway estuvo offline y su certificado expiró, el
flujo de renovación cae de vuelta al canje de un código de activación nuevo (misma
mecánica de reemplazo del Flujo 7, sin cambiar `site`/`cameras`).
...
* **Infraestructura Cloud (AWS/Supabase):** balanceo, backend, PostgreSQL
  gestionado con particionamiento nativo + `pg_partman` y tiering del histórico
  frío a S3 Glacier (Sección 8.6, lo que hace sostenible la retención de 13 meses
  sin la compresión columnar de Timescale), Model Registry (S3 + tráfico de descarga de
...
### Apéndice A — Changelog (v3.3 arriba; iteraciones previas debajo)
